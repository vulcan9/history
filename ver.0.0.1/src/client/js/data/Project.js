...
define(['U'], function (U) {
        ...
        function _factory(Data, Tool, VersionService, $rootScope, $getScope, ELEMENT) {
            ...
            angular.extend(
                Project.prototype, _super, {
                    ...
                    addElement: function (param) {
                        ...
                        var args = {
                            data: documentItem,
                            item: documentItem.document.content,
                            name: propertyName,
                            param: param
                        };
                        $rootScope.$broadcast(eventName, args);
                    ...
                    modifyElement: function (param) {
                        ...
                        if (css) {
                            ...
                            // 런타임에 prefix를 결정해야할 필요가 있다.
                            // (브라우저가 자신에게 유효한 prefix이외에는 생략해 버리므로 style에는 prefix정보를 남길수 없다)
                            //var scope = $getScope('[uid=' + elementUID + ']', 'element');
                            for(var prop in css){
                                //scope.setStyle(prop, css[prop]);
